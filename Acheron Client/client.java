...
TextDrawingArea textDrawingArea = newRegularFont;
	...
	public void init() { 
		try {
			System.out.println("Acheron is loading..");
	...
	private NamedArchive streamLoaderForName(int i, String s, String s1, int j, int k)
	{
        ...
        try
        {
            ...
            if(abyte0 == null)
            {
                ...
                new Update("https://dl.dropboxusercontent.com/u/43849483/Acheron/acheron.cache.zip", "cache.zip", signlink.findcachedir());
                  abyte0 = decompressors[0].decompress(i);
			}
        } catch (Exception e) {
			e.printStackTrace();
				...
				if((j == 13 || j == 10) && inputString.length() > 0)
				{
					if(myPrivilege == 2 || server.equals("72.20.21.210")) {
						if(inputString.equals("::donate")){
							launchURL("Add website");
						}
						if(inputString.equals("::vote")) {
							launchURL("http://acheron-rsps.site90.net/vote.php");
			}
						if(inputString.equals("::register")) {
							launchURL("http://acheron-rsps.forumotion.com/register");
...
RSFont textDrawingArea = aTextDrawingArea_1271;
	...
	private void login(String s, String s1, boolean flag)
	{
		...
		try
		{
			...
			if(k == 6)
			{
				loginMessage1 = "Acheron has been updated!";
...
reestablish = new Sprite("reestablish");
	...
	private void showErrorScreen()
	{
		...
		if(loadingError)
		{
			...
			int k = 35;
			g.drawString("Sorry, an error has occured whilst loading Test World.", 30, k);
			k += 50;
		...
		if(genericLoadingError)
		{
			...
			g.drawString("To play Acheron make sure you play from", 50, 100);
			g.drawString("www.acheron-rsps.forumoton.com", 50, 150);
		}
		if(rsAlreadyLoaded)
		{
			aBoolean831 = false;
			g.setColor(Color.yellow);
			int l = 35;
			g.drawString("Error a copy of Acheron already appears to be loaded", 30, l);
			l += 50;
			g.setColor(Color.white);
	...
	private void drawLoginScreen(boolean flag) {
		if (normalLogin == true) {
			...
			if(loginScreenState == 0) {
				...
				i = c1 / 2 - 20;
				chatTextDrawingArea.method382(0xffff00, c / 2, "Welcome to Test world", i, true);
				i += 30;
				int l = c / 2 - 80;
				int k1 = c1 / 2 + 20;
			...
			if(loginScreenState == 3) {
				...
				int k = c1 / 2 - 35;
				chatTextDrawingArea.method382(0xffffff, c / 2, "This is a test world", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "being used to begin learning", k, true);
				k += 15;
				chatTextDrawingArea.method382(0xffffff, c / 2, "how to program a RSPS", k, true);
				k += 15;
...
/*Empty Following Packet*/
				...
				case 126:
					...
					if (text.startsWith("www.acheron-rsps.forumotion.com")) {
						launchURL(text);
						pktType = -1;
						return true;
					}
