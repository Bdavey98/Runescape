...
return 15;
case 14936://agile top
case 14938://agile legs
return 100;
case 19311://3rd age druidic cape
return 50;
...
public boolean addShopItem(int itemID, int amount) {
	...
	public boolean buyItem(int itemID, int fromSlot, int amount) {
		...
		if (amount > 0) {
			...
			for (int i = amount; i > 0; i--) {
                ...
                if(c.myShopId == 29 || c.myShopId == 30 || c.myShopId == 31) {
                	...
                	if (c.survivalPoints >= TotPrice2) {
						if (c.getItems().freeSlots() > 0) {
							buyDelay = System.currentTimeMillis();
							c.survivalPoints -= TotPrice2;
							c.getItems().addItem(itemID, 1);
							Server.shopHandler.ShopItemsN[c.myShopId][fromSlot] -= 1;
							Server.shopHandler.ShopItemsDelay[c.myShopId][fromSlot] = 0;
							if ((fromSlot + 1) > Server.shopHandler.ShopItemsStandard[c.myShopId]) {
		...
		public void handleOtherShop(int itemID) {
			...
			if (c.myShopId == 91) {
				if (c.survivalPoints >= getSpecialItemValue(itemID)) {
					if (c.getItems().freeSlots() > 0){
						c.survivalPoints -= getSpecialItemValue(itemID);
						c.getItems().addItem(itemID,1);
						c.getItems().resetItems(3823);
					}
				} else {
