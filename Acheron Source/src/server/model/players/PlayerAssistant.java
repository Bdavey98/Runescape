...
public class PlayerAssistant {
	public boolean teleTabTeleport(int x, int y, int height, String teleportType) {
		...
		} else if (c.inSurvival) {
			c.sendMessage("You can't teleport out of Survival!");
	...
	public void giveLife() {
		...
		if (c.duelStatus <= 4 && !c.getPA().inPitsWait()) { // if we are not in a duel we must be in wildy so remove
															// items
			if (!c.inPits && !c.inFightCaves() && !c.inPcGame() && !c.inFunPk() && !c.inSurvival) {
	...
	public void startTeleport(int x, int y, int height, String teleportType) {
		...
		if (c.inSurvival) {
			c.sendMessage("You cannot teleport out of Survival!");
			return;
		}
	...
	public static int ChestRewards[] = { 15068, 15067, 15066, 15063, 15065, 15064, 15061, 15062, 15060 };
	...
	// resets waveID, number you've killed and number you had to kill
	// moves player back to main area and resets booleans
	...
	// moves player inside survival area at a unique height level
	// sets values for start of survival
	// starts the spawning of the waves
	public void enterSurvival() {
		c.getPA().movePlayer(2766, 9632, c.playerId * 12);
		c.waveId = 0;
		c.survivalToKill = -1;
		c.survivalKilled = -1;
		c.waveSpawn();
