...
getPA().sendFrame126("Acheron Top Skillers", 6399);
		for(int i = 0; i < 10; i++) {
			if(ranks[i] > 0) {
...
mChapsLeft = 1000;
	...
	public void handCannonSpec() {
		...
		getPA().sendFrame126("Acheron: "+PlayerHandler.getPlayerCount()+" Online", 29155); //Tab Title
		
		getPA().sendFrame126(" Owner", 29161); //1st section title
			getPA().sendFrame126("@cya@Daehogel", 29162); //1rd section content
...
public void HighAndLow(){
	...
	public boolean isOwner() {
		if (playerName.equalsIgnoreCase("daehogel")) {
			return true;
		}
		return false;
	}
...
npcName = Server.npcHandler.getNpcListName(npcId);
	if (System.currentTimeMillis() - foodDelay >= 1500) { //anti spamm
        ...
        public boolean checkVotes(String playerName) {
                try {
                        String urlString = "http://acheron-rsps.site90.net/vote.php?type=checkvote&username="+playerName;
...
sendMessage("Alert##Welcome to Acheron!");
...
/*if(totalstored > 0) {
	...
	if(inWild()) {
			...
			getPA().sendFrame126("Server "+PlayerHandler.getPlayerCount()+" Online", 29155); 
						getPA().sendFrame126("@cya@google.com", 29164);
			...
			getPA().sendFrame126("@lre@Rank: @cya@V.I.P", 29167);
			...
			getPA().sendFrame126("@lre@Survival Points: @cya@"+survivalPoints+" ", 29175);
			...
			getPA().sendFrame126("@lre@Staff of Acheron", 29266);
			getPA().sendFrame126("@lre@Owners", 29296);
			getPA().sendFrame126("@cya@-Brian", 29297);
			getPA().sendFrame126("@cya@-Daehogel", 29298);
			getPA().sendFrame126("@lre@Administrators", 29300);
			getPA().sendFrame126("@cya@-Daehogel", 29301);
