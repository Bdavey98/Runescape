...
p.lastConnectedFrom.add(token2); 
					...
					} else if (token.equals("survivalPoints")) {
						p.survivalPoints = Integer.parseInt(token2);
...
characterfile.newLine();
			...
			characterfile.newLine();
			characterfile.write("survivalPoints = ", 0, 17);
			characterfile.write(Integer.toString(p.survivalPoints), 0, Integer.toString(p.survivalPoints).length());
			characterfile.newLine();
			characterfile.newLine();
