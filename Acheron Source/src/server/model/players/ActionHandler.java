...
case 2471:
		...
		// commented so i could use for survival
		// case 13623: 
		// c.getPA().movePlayer(2837, 3806, 0);
...
break;
	...
	Dungeoneering Cases!
		...
		case 13901:
		    if (c.Zammy < 12) {
				...
				c.sendMessage("Current kills: " + c.Zammy);
				c.isSkulled = true;
		    return;
		    }	
			...
			c.sendMessage("You recieve no supplies for your battle!");
			c.autoRet = 0;
			c.getPA().refreshSkill(3);
			c.getPA().refreshSkill(0);
			c.playerLevel[3] = 99;
		...
		if (System.currentTimeMillis() - c.lastEntrance < 10000) { // permit entrance every 10 sec
			...
			c.inEasySurvival = true;
			c.inSurvival = true;
			c.inMediumSurvival = false;
			c.inHardSurvival = false;
			c.sendMessage("Survival is about to start. Prepare your defenses.");
			c.waveID = 0;
			c.getPA().enterSurvival();
			c.lastEntrance = System.currentTimeMillis();
		}
		...
		case 6821:
			...
			if(c.barrowsNpcs[5][1] == 0) {
				Server.npcHandler.spawnNpc(c, 2025, c.getX()-1, c.getY()-1, -1, 0, 90, 19, 200, 200, true, true);
				c.barrowsNpcs[5][1] = 1;
			} else {
				c.sendMessage("You have already searched in this sarcophagus.");
			}
		...
		case 172:
			  if(!c.getItems().playerHasItem(989, 1)) {
				  c.sendMessage("A Crystal Key is required to open this chest.");
			  break;
			  } else {
	...
	public void secondClickObject(int objectType, int obX, int obY) {
		...
		switch (objectType) {
			...
			case 4874:
				c.getThieving().stealFromStall(315, 10, 1);
				break;
			case 4875:
		...
		switch (npcType) {
			...
			case 2588:
				...
				if (System.currentTimeMillis() - c.lastHeal < 600000) { // permit healing every 10 min
					c.sendMessage("You can only heal every 10 minutes!");
				} else if (c.playerLevel[3] != c.getLevelForXP(c.playerXP[3])) {
					c.playerLevel[3] += 100;
					if (c.playerLevel[3] > c.getLevelForXP(c.playerXP[3])) // makes sure doesnt heal past max
						c.playerLevel[3] = c.getLevelForXP(c.playerXP[3]);
					c.getPA().refreshSkill(3);
					c.lastHeal = System.currentTimeMillis();
					c.sendMessage("You have been healed");
				} else {
					c.sendMessage("You are already at full hp!");
				}
				break;
